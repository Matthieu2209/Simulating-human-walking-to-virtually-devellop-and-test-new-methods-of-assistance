...
# Stimulations
StimL = np.array([0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01])
StimR = np.array([0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01])
...
def user_JointForces(mbs_data, tsim):
    ...
    if tsim == 0 :
        Ldx = u_f.low_filter(dx_innerjointL, 0.02, 0, Ldx_prec)
        Rdx = u_f.low_filter(dx_innerjointR, 0.02, 0, Rdx_prec)
        # MBsysPy.set_output_value(Ldx, 1, "Ldx_Rdx_filter")
        # MBsysPy.set_output_value(Rdx, 2, "Ldx_Rdx_filter")
    else: 
        if dt!=0 :
    ...
    if tsim == 0 :
        ...
        Fm_VAS_L = muscle.vce_compute(lce_VAS_L, lmtu_VAS_L, A_VAS_L, VAS)[1]
        Fm_VAS_R = muscle.vce_compute(lce_VAS_R, lmtu_VAS_R, A_VAS_R, VAS)[1]
        # MBsysPy.set_output_value(muscle.vce_compute(lce_VAS_L, lmtu_VAS_L, A_VAS_L, VAS)[0], 1, "Vce_VAS")
        # MBsysPy.set_output_value(muscle.vce_compute(lce_VAS_L, lmtu_VAS_L, A_VAS_L, VAS)[1], 1, "Fm_VAS")
